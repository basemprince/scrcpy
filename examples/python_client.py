            ...
            for event in pygame.event.get():
                ...
                if event.type in (pygame.KEYDOWN, pygame.KEYUP):
                    ...
                    if mods & (pygame.KMOD_LALT | pygame.KMOD_LMETA) and not (mods & pygame.KMOD_SHIFT) and not event.repeat:
                        if event.key == pygame.K_h:
                            client.state.control.inject_keycode(3, action)  # HOME
                            continue
                        if event.key in (pygame.K_b, pygame.K_BACKSPACE):
                            client.state.control.back_or_screen_on(action)
                            continue
                        if event.key == pygame.K_s:
                            client.state.control.inject_keycode(187, action)  # APP_SWITCH
                            continue
                        if event.key == pygame.K_m:
                            client.state.control.inject_keycode(82, action)  # MENU
                            continue
                        if event.key == pygame.K_p:
                            client.state.control.inject_keycode(26, action)  # POWER
                            continue
                        if event.key == pygame.K_n:
                            if mods & pygame.KMOD_SHIFT:
                                client.state.control.collapse_panels()
                            elif event.type == pygame.KEYDOWN:
                                client.state.control.expand_notification_panel()
                            continue
                ...
                if event.type in (pygame.MOUSEBUTTONDOWN, pygame.MOUSEBUTTONUP):
                    ...
                    if button is not None:
                        ...
                        if down:
                            client.state.control.mouse_buttons |= button
                        else:
                            client.state.control.mouse_buttons &= ~button
