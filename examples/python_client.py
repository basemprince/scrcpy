...
class Client:
    ...
    def _start_server(self) -> None:
        subprocess.run(self.adb_cmd + ["push", self.server, DEVICE_SERVER_PATH], check=True)
                ...
                while True:
                    ...
                    for frame in decoder.decode(packet):
                        if pygame_available:
                            ...
                            self.last_frame = img
                            cv2.imshow("scrcpy", img)
                            if cv2.waitKey(1) & 0xFF == ord("q"):
                                return
                        else:
                            self.last_frame = frame.to_ndarray(format="rgb24")
                            print(
                                f"Received frame: pts={frame.pts} size={frame.width}x{frame.height}"
                            )
        finally:
            self._stop_server()
            if cv2:
